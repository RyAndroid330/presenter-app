...
/* Presenter area — shrinks naturally when sidebar expands */
.presenter-area {
  flex: 1 1 0;
  min-width: 0;
  height: 100%;
...
/* ── Right sidebar ── */
.right-sidebar {
  flex: 0 0 52px;         /* icon-strip only width by default */
  height: 100%;
  display: flex;
  flex-direction: row;    /* icons left, panel right */
...
/* When a panel is open, sidebar grows to icon + panel width */
.right-sidebar.panel-open {
  flex: 0 0 352px;        /* 52px icons + 300px panel */
}

/* Vertical icon strip — always 52px, never shrinks */
.sidebar-icons {
  flex: 0 0 52px;
  display: flex;
  flex-direction: column;
...
/* Panel — takes remaining width next to icon strip */
.sidebar-panels {
  flex: 1;
  min-width: 0;
  height: 100%;
...
@media (max-width: 768px) and (orientation: portrait), (max-width: 500px) {
  ...
  /* On mobile, sidebar stacks vertically — icons at bottom, panel above */
  .right-sidebar,
  .right-sidebar.panel-open {
    flex: none;
    width: 100%;
